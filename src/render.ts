...
function roomTitleOf(task: MentionTask): string {
  return task.payload.postTitle || task.postId || "未命名房间";
}

function senderOf(task: MentionTask): string {
  return task.payload.mentionedByDisplayName || task.payload.mentionedByUserOneId || "房间成员";
}

function messageContentOf(task: MentionTask): string {
...
function inferSubjectLabel(task: MentionTask, roomMessages: BuddyRoomMessage[]): string {
  const corpus = [
...
export function buildRoomSessionKey(baseSessionKey: string, postId: string): string {
...
export function buildRoomSessionLabel(task: MentionTask): string {
...
export function selectRoomMessagesForContext(
  task: MentionTask,
  ...
  for (let index = normalized.length - 1; index >= 0; index -= 1) {
    ...
    if (
      (currentMessageId && String(item.id || "").trim() === currentMessageId)
  ...
  const currentMessageExists = bounded.some(
    (item) =>
      (currentMessageId && String(item.id || "").trim() === currentMessageId)
      || (currentMessageContent && String(item.content || "").trim() === currentMessageContent),
  );
...
function formatRoomMessagesContext(roomMessages: BuddyRoomMessage[]): string {
...
function buildDirectStrategyReply(
  intent: ReplyIntent,
  subject: string,
  task: MentionTask,
): string | null {
...
export function buildTaskInjectionText(
  task: MentionTask,
  roomMessages: BuddyRoomMessage[] = [],
): string {

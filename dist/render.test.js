...
import { buildBootstrapMessage, buildReplyStrategy, buildRoomSessionKey, buildAutoReplyPrompt, buildFallbackReply, buildTaskInjectionText, extractChatReplyText, selectRoomMessagesForContext, sanitizeModelReplyText, } from "./render.js";
describe("render helpers", () => {
    ...
    it("builds injection text and fallback reply", () => {
        ...
        expect(buildFallbackReply(task)).toBe("看到了。你可以直接补一句最想让我判断的点，我就按这条继续给你建议。");
    });
    it("builds room-scoped session keys", () => {
        expect(buildRoomSessionKey("agent:main:chek:mentions", "POST-1 Demo")).toBe("agent:main:chek:mentions:room:post-1-demo");
    });
    ...
    it("appends the current message into room context when missing", () => {
        ...
        const context = selectRoomMessagesForContext(task, []);
